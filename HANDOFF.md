...
| 파일 | 역할 |
|------|------|
| `app/api/search/route.js` | 법제처 API 검색 핵심 로직 (Phase 2 복원됨) |
...
### 완료
- 법제처 API 인증 해결 (OC=1234 + Referer 헤더 + Edge Runtime)
- Phase 3 롤백: route.js 161줄 → 84줄 (Phase 2 순수 로직 복원)
- PROGRESS.md + HANDOFF.md 최신화

### 다음 작업 (Phase 3)
- 주제어 검색 개선 (예: "진료기록부" → 관련 조문 반환)
  - 현재: lawSearch.do는 법령명만 검색 → 주제어 결과 없음
  - 방향: 의료법 관련 법령 ID 하드코딩 후 직접 조문 검색
- AI 요약 기능 (Claude API 또는 OpenAI API)
- UI/UX 개선
...
| Phase 3 처럼 검색이 망가짐 | 복잡한 로직 추가 후 부작용 | route.js를 Phase 2 로직으로 롤백 (84줄 버전) |

---

*마지막 업데이트: 2026-03-26*# HANDOFF.md — MdLaw 프로젝트 인수인계 가이드

...
export async function GET(request) {
  try {
    ...
    try {
      ...
      console.error("[/api/search] lawSearch.do 호출 실패:", fetchErr.message);
      return NextResponse.json({ error: `법제처 연결 실패: ${fetchErr.message}` }, { status: 500 });
    }

    const lawList = toArray(searchData?.LawSearch?.law);
    console.log(`[/api/search] keyword="${keyword}" → 법령 ${lawList.length}건`);
    ...
    for (const law of lawList.slice(0, 3)) {
      ...
      const lawName = typeof rawLawName === "object"
        ? String(rawLawName["#text"] || rawLawName.text || Object.values(rawLawName)[0] || "")
        : String(rawLawName);
      ...
      try {
        ...
        console.error(`[/api/search] lawService.do 호출 실패 (lawId=${lawId}):`, err.message);
        continue;
      }
      ...
      console.log(`[/api/search] lawId=${lawId} "${lawName}" → 조문 ${articles.length}건`);

      // 1차: 키워드 매칭 조문
      let matched = articles.filter((a) => {
        const fullText = extractFullText(a).toLowerCase();
        return fullText.includes(kw);
      }).slice(0, 3);

      // 2차 fallback: 키워드 매칭 없으면 첫 3개 조문
      if (!matched.length && articles.length > 0) {
        console.log(`[/api/search] 키워드 매칭 없음 → fallback 첫 3개 사용`);
    ...
    console.log(`[/api/search] 최종 결과: ${results.length}건`);

  ...
  return (
    <div className="flex flex-col items-center gap-8 pt-8 pb-16">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm">병원 실무자를 위한 의료법령 검색 AI 탐색기</p>
      </div>
      ...
      <div className="w-full max-w-xl">
        <div className="flex gap-2 shadow-md rounded-xl overflow-hidden border border-gray-200 bg-white">
          ...
          />
          <button onClick={() => handleSearch()} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 text-sm font-medium transition-colors">
            검색
          </button>
        </div>
        <p className="text-xs text-gray-400 mt-2 text-center">실무 표현 그대로 입력하세요. 예: &quot;차트 보관&quot; → 진료기록부 보존기간으로 자동 변환</p>
      </div>

      {/* 자주 찾는 주제 — 추후 버튼 추가 예정 */}
      <div className="w-full max-w-xl">
        <p className="text-xs text-gray-400 mb-2 font-medium">자주 찾는 주제</p>
        <div className="bg-white border border-dashed border-gray-300 rounded-xl p-4 text-center text-sm text-gray-400">주제 버튼은 추후 추가 예정입니다</div>
      </div>
